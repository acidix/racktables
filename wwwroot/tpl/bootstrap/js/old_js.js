...
$(window).bind('popstate', function(event) {
    ...
    if (state) {
        document.body.load(state.path);
    }
});

(function($) {
    $(document).ready(function() {
        console.log('Here is document ready again');
        prepareContent();
    });
})(jQuery);
...
function prepareContent(selector) {
    ...
    // Set svgs
    if($$('.rackspace-svg-container').length > 0)
        svgPanZoom('.rackspace-svg-container');

   /* if($$('.tree-view-list').length > 0)
       $$('.tree-view-list').jstree({
           "core": { "themes" : {  "icons" : false } },
           "plugins" : [ "wholerow", "checkbox" ]
        });
    */
   // $$('svg').svgPan('class="svg-pan-zoom_viewport);
...
function setupTabs(selector) {
    ...
    $(selector).find('li.tab').click(function (event) {
        event.preventDefault();
...
function setupSiteViewStack() {
    ...
    $('#contentarea > .remove-view-btn').addClass('btn-no-border');
...
function checkAjaxHrefBtn() {
    for(var i = 0; i < $('button.ajax_href').length; i++) {
        ...
        btn.click(function (event) {
            $.get(window.location.pathname + $(this).attr('href')).done(
                function (data) {
                    bodytxt = '<div id="body-mock">' + data.replace(/^[\s\S]*<body.*?>|<\/body>[\s\S]*$/g, '') + '</div>';
                    var bodyelem = $(bodytxt);
                    //console.log(data);
...
function checkAjaxFormBtn() {
    for(var i = 0; i < $('button.ajax_form').length; i++) {
        ...
        var form = $($('button.ajax_form').eq(i).attr('targetform'));
        ...
        form.submit(function (e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // Issue an event to the messenger
                    document.bootstrap_template.event_messenger.push({ name: 'ajax_reguest', success : bodyelem.find('.alert.alert-success').length > 0});
                    // Setting conentarea new if success if not set
                    if(bodyelem.find('.content').length > 0) {
                        $('.content').html(bodyelem.find('.content').html());
                    } else {
                        addAlertMessage($('<div class="alert alert-danger"></div>').append(bodyelem));
